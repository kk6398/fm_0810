...
from ..export.flowmap2gs import flowmap_2_gs, save_json
...
class ModelWrapperOverfit(LightningModule):
    ...
    def training_step(self, dummy):
        choice = 2          # 选择模式, 1:flowmap      2:colmap
        ...
        if self.global_step >= 2000:      # 从第2000次迭代进行3dgs render
            if choice == 1:
        ...
        if self.global_step > 2000:  # 因为self.global_step从0开始
            self.gaussians.update_learning_rate((self.global_step-2000))  # 根据当前迭代次数更新学习率
            # lr = self.gaussians.update_learning_rate(self.global_step)
            # self.log("train/loss/learning_rate", lr)
        # Every 1000 its we increase the levels of SH up to a maximum degree
        if self.global_step % 1000 == 0 and self.global_step > 2000:  # 每1000次迭代，提升球谐函数的次数以改进模型复杂度     # 初始为0阶
            self.gaussians.oneupSHdegree()

        # 选择1 点云通过unproject生成，并进一步生成render的gaussian参数
        if self.global_step >= 2000:      # 2000
            if self.global_step == 2000:  # 2000
                if choice == 1:
        ...
        if (self.global_step > 2000) and (self.global_step <= 7000):     # 2000~7000次迭代，每200次迭代进行一次测试
            if (self.global_step % 200 == 0 and self.global_step > 2000):   # 0
